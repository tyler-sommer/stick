...
// parseFunc parses a function call expression from the opening parenthesis until the closing parenthesis.
func (t *Tree) parseFunc() (Expr, error) {
	tok, err := t.expect(tokenName)
	if err != nil {
		return nil, err
	}
	_, err = t.expectValue(tokenParensOpen, "(")
	if err != nil {
		return nil, err
	}
